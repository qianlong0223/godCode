...
- (JLBaseAPI *)dealWhileSuccess:(id)responseObject {
    ...
    if (!responseObject) {
        ...
        // token过期，直接退出
        CLog(@"token过期");
        [StateView showWarningInfo:[NSString stringWithFormat:@"请求失败:%@",info]];
        [JLProgressHUD hide];
        return nil;
    }
    else if (code == NetworkCodeTypeFail) {
        ...
        [PPNetworkHelper GET:self.subUrl parameters:self.parameters success:^(id responseObject) {
            if([[responseObject allKeys] containsObject:@"data"]){
                if ([[responseObject objectForKey:@"data"] isKindOfClass:[NSDictionary class]]) {
                    ...
                    [self requestSucces:data successJCDataHandler:dataHandler];
                }
