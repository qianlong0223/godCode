...
+ (NSString *)timeDifferenceWithNowTimer:(NSString *)timerSp {
    ...
    NSTimeInterval time = currentTime - createTime;
     ...
     //秒转分钟
     if (small < 60) {
         return [NSString stringWithFormat:@"用时%ld分钟",(long)small];
     }
     
     // 秒转小时
     NSInteger hours = time/3600;
     if (hours<24) {
         return [NSString stringWithFormat:@"用时%ld小时",(long)hours];
     }
     //秒转天数
     NSInteger days = time/3600/24;
     if (days < 30) {
         return [NSString stringWithFormat:@"用时%ld天",(long)days];
     }
     //秒转月
     NSInteger months = time/3600/24/30;
     if (months < 12) {
         return [NSString stringWithFormat:@"用时%ld月",(long)months];
     }
     //秒转年
     NSInteger years = time/3600/24/30/12;
    return [NSString stringWithFormat:@"用时%ld年",(long)years];
